...
const RecipeItem = ({item}) => {
  return (
    <div className='flex flex-col w-80 overflow-hidden p-5 shadow-xl gap-5 border-2 rounded-2xl border-white '>
     ...
     <Link to={`/recipe-item/${item?.id}`} className='text-sm p-3 rounded-lg uppercase font-medium tracking-wider inline-block shadow-md bg-black text-amber-50 '>Recipe Details</Link>
    </div>
  )
}
